 ...
 */
public class IntValueHashMap<K> extends AbstractIntValueMap<K>
        implements IntValueMap<K>, Cloneable, Serializable {
     ...
     */
    public IntValueHashMap(Map<? extends K, Integer> m) {
    ...
    public IntValueHashMap(IntValueMap<K> m) {
        this(Math.max((int) (m.size() / DEFAULT_LOAD_FACTOR) + 1, DEFAULT_INITIAL_CAPACITY),
                DEFAULT_LOAD_FACTOR);
        inflateTable(threshold);
    ...
    public int get(Object key, int defaultrRetVal) {
        if (key == null)
     ...
     */
    private int getForNullKey(int defaultRetVal) {
        if (size == 0) {
    ...
    public int put(K key, int value, int defaultRetVal) {
     ...
     */
    private int putForNullKey(int value, int defaultRetVal) {
        for (Entry<K> e = table[0]; e != null; e = e.next) {
            if (e.key == null) {
                int oldValue = e.value;
                e.value = value;
    ...
    private void putAllForCreate(Map<? extends K, Integer> m) {
        for (Map.Entry<? extends K, Integer> e : m.entrySet())
            putForCreate(e.getKey(), e.getValue());
    }

    private void putAllForCreate(IntValueMap<? extends K> m) {
        for (IntValueMap.Entry<? extends K> e : m.entrySet())
            putForCreate(e.getKey(), e.getValue());
    }
    ...
    public void putAll(IntValueMap<K> m) {
        int numKeysToBeAdded = m.size();
        if (numKeysToBeAdded == 0)
            return;
    ...
    public int remove(Object key, int defaultRetVal) {
    ...
    final Entry<K> removeMapping(Object o) {
        if (size == 0 || !(o instanceof IntValueMap.Entry))
            return null;

        @SuppressWarnings("unchecked")
        IntValueMap.Entry<K> entry = (IntValueMap.Entry<K>) o;
    ...
    @SuppressWarnings("unchecked")
    public Object clone() {
        IntValueHashMap<K> result = null;
        try {
            result = (IntValueHashMap<K>) super.clone();
        ...
        if (result.table != EMPTY_TABLE) {
            result.inflateTable(Math.min((int) Math.min(size * Math.min(1 / loadFactor, 4.0f),
                    // we have limits...
                    IntValueHashMap.MAXIMUM_CAPACITY), table.length));
        }
    ...
    static class Entry<K> implements IntValueMap.Entry<K> {
        final K key;
        int value;
        Entry<K> next;
        int hash;
        ...
        public final boolean equals(Object o) {
            if (!(o instanceof IntValueMap.Entry))
                return false;
            @SuppressWarnings("rawtypes")
            IntValueMap.Entry e = (IntValueMap.Entry) o;
            Object k1 = getKey();
            Object k2 = e.getKey();
        ...
        public final int hashCode() {
            return Objects.hashCode(getKey()) ^ Helper.intHash(getValue());
        }
         ...
         * HashMap.
         */
        void recordAccess(IntValueHashMap<K> m) {
        }

        /**
         * This method is invoked whenever the entry is removed from the table.
         */
        void recordRemoval(IntValueHashMap<K> m) {
        }
    }
    ...
    private abstract class HashIterator {
        ...
        public void remove() {
            ...
            current = null;
            IntValueHashMap.this.removeEntryForKey(k);
            expectedModCount = modCount;
        }
    }

    private final class ValueIterator extends HashIterator implements IntIterator {
        public int next() {
    ...
    private final class EntryIterator extends HashIterator
            implements Iterator<IntValueMap.Entry<K>> {
        public IntValueMap.Entry<K> next() {
            return nextEntry();
        }
    }
    ...
    Iterator<IntValueMap.Entry<K>> newEntryIterator() {
        return new EntryIterator();
    }

    // Views

    private transient Set<IntValueMap.Entry<K>> entrySet = null;
    private transient volatile Set<K> keySet = null;
    private transient volatile IntCollection values = null;
    ...
    private final class KeySet extends AbstractSet<K> {
        ...
        public boolean remove(Object o) {
            return IntValueHashMap.this.removeEntryForKey(o) != null;
        }
     ...
     */
    public Set<IntValueMap.Entry<K>> entrySet() {
        return entrySet0();
    }

    private Set<IntValueMap.Entry<K>> entrySet0() {
        Set<IntValueMap.Entry<K>> es = entrySet;
        return es != null ? es : (entrySet = new EntrySet());
    }

    private final class EntrySet extends AbstractSet<IntValueMap.Entry<K>> {
        public Iterator<IntValueMap.Entry<K>> iterator() {
        ...
        public boolean contains(Object o) {
            if (!(o instanceof IntValueMap.Entry))
                return false;
            @SuppressWarnings("unchecked")
            IntValueMap.Entry<K> e = (IntValueMap.Entry<K>) o;
    ...
    private void writeObject(java.io.ObjectOutputStream s) throws IOException {
        ...
        if (size > 0) {
            for (IntValueMap.Entry<K> e : entrySet0()) {
    ...
    private static final long serialVersionUID = -232108156025330878L;
