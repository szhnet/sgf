 ...
 */
public class LongValueHashMap<K> extends AbstractLongValueMap<K>
        implements LongValueMap<K>, Cloneable, Serializable {
     ...
     */
    public LongValueHashMap(Map<? extends K, Long> m) {
    ...
    public LongValueHashMap(LongValueMap<K> m) {
        this(Math.max((int) (m.size() / DEFAULT_LOAD_FACTOR) + 1, DEFAULT_INITIAL_CAPACITY),
                DEFAULT_LOAD_FACTOR);
        inflateTable(threshold);
    ...
    public long get(Object key, long defaultrRetVal) {
        if (key == null)
     ...
     */
    private long getForNullKey(long defaultRetVal) {
        if (size == 0) {
    ...
    public long put(K key, long value, long defaultRetVal) {
     ...
     */
    private long putForNullKey(long value, long defaultRetVal) {
        for (Entry<K> e = table[0]; e != null; e = e.next) {
            if (e.key == null) {
                long oldValue = e.value;
                e.value = value;
    ...
    private void putAllForCreate(Map<? extends K, Long> m) {
        for (Map.Entry<? extends K, Long> e : m.entrySet())
            putForCreate(e.getKey(), e.getValue());
    }

    private void putAllForCreate(LongValueMap<? extends K> m) {
        for (LongValueMap.Entry<? extends K> e : m.entrySet())
            putForCreate(e.getKey(), e.getValue());
    }
    ...
    public void putAll(LongValueMap<K> m) {
        int numKeysToBeAdded = m.size();
        if (numKeysToBeAdded == 0)
            return;
    ...
    public long remove(Object key, long defaultRetVal) {
    ...
    final Entry<K> removeMapping(Object o) {
        if (size == 0 || !(o instanceof LongValueMap.Entry))
            return null;

        @SuppressWarnings("unchecked")
        LongValueMap.Entry<K> entry = (LongValueMap.Entry<K>) o;
    ...
    @SuppressWarnings("unchecked")
    public Object clone() {
        LongValueHashMap<K> result = null;
        try {
            result = (LongValueHashMap<K>) super.clone();
        ...
        if (result.table != EMPTY_TABLE) {
            result.inflateTable(Math.min((int) Math.min(size * Math.min(1 / loadFactor, 4.0f),
                    // we have limits...
                    LongValueHashMap.MAXIMUM_CAPACITY), table.length));
        }
    ...
    static class Entry<K> implements LongValueMap.Entry<K> {
        final K key;
        long value;
        Entry<K> next;
        int hash;
        ...
        public final boolean equals(Object o) {
            if (!(o instanceof LongValueMap.Entry))
                return false;
            @SuppressWarnings("rawtypes")
            LongValueMap.Entry e = (LongValueMap.Entry) o;
            Object k1 = getKey();
            Object k2 = e.getKey();
        ...
        public final int hashCode() {
            return Objects.hashCode(getKey()) ^ Helper.longHash(getValue());
        }
         ...
         * HashMap.
         */
        void recordAccess(LongValueHashMap<K> m) {
        }

        /**
         * This method is invoked whenever the entry is removed from the table.
         */
        void recordRemoval(LongValueHashMap<K> m) {
        }
    }
    ...
    private abstract class HashIterator {
        ...
        public void remove() {
            ...
            current = null;
            LongValueHashMap.this.removeEntryForKey(k);
            expectedModCount = modCount;
        }
    }
    ...
    private final class EntryIterator extends HashIterator
            implements Iterator<LongValueMap.Entry<K>> {
        public LongValueHashMap.Entry<K> next() {
            return nextEntry();
        }
    }
    ...
    Iterator<LongValueMap.Entry<K>> newEntryIterator() {
        return new EntryIterator();
    }

    // Views

    private transient Set<LongValueMap.Entry<K>> entrySet = null;
    private transient volatile Set<K> keySet = null;
    private transient volatile LongCollection values = null;
    ...
    private final class KeySet extends AbstractSet<K> {
        ...
        public boolean remove(Object o) {
            return LongValueHashMap.this.removeEntryForKey(o) != null;
        }
     ...
     */
    public Set<LongValueMap.Entry<K>> entrySet() {
        return entrySet0();
    }

    private Set<LongValueMap.Entry<K>> entrySet0() {
        Set<LongValueMap.Entry<K>> es = entrySet;
        return es != null ? es : (entrySet = new EntrySet());
    }

    private final class EntrySet extends AbstractSet<LongValueMap.Entry<K>> {
        public Iterator<LongValueMap.Entry<K>> iterator() {
        ...
        public boolean contains(Object o) {
            if (!(o instanceof LongValueMap.Entry))
                return false;
            @SuppressWarnings("unchecked")
            LongValueMap.Entry<K> e = (LongValueMap.Entry<K>) o;
    ...
    private void writeObject(java.io.ObjectOutputStream s) throws IOException {
        ...
        if (size > 0) {
            for (LongValueMap.Entry<K> e : entrySet0()) {
    ...
    private static final long serialVersionUID = -4614853317841172752L;
