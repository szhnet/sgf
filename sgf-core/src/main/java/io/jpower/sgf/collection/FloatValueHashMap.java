...
public class FloatValueHashMap<K> extends AbstractFloatValueMap<K>
        implements FloatValueMap<K>, Cloneable, Serializable {
     ...
     */
    public FloatValueHashMap(Map<? extends K, Float> m) {
    ...
    public FloatValueHashMap(FloatValueMap<K> m) {
        this(Math.max((int) (m.size() / DEFAULT_LOAD_FACTOR) + 1, DEFAULT_INITIAL_CAPACITY),
                DEFAULT_LOAD_FACTOR);
        inflateTable(threshold);
    ...
    public float get(Object key, float defaultrRetVal) {
        if (key == null)
     ...
     */
    private float getForNullKey(float defaultRetVal) {
        if (size == 0) {
    ...
    public float put(K key, float value, float defaultRetVal) {
     ...
     */
    private float putForNullKey(float value, float defaultRetVal) {
        for (Entry<K> e = table[0]; e != null; e = e.next) {
            if (e.key == null) {
                float oldValue = e.value;
                e.value = value;
    ...
    private void putAllForCreate(Map<? extends K, Float> m) {
        for (Map.Entry<? extends K, Float> e : m.entrySet())
            putForCreate(e.getKey(), e.getValue());
    }

    private void putAllForCreate(FloatValueMap<? extends K> m) {
        for (FloatValueMap.Entry<? extends K> e : m.entrySet())
            putForCreate(e.getKey(), e.getValue());
    }
     ...
     * @throws NullPointerException if the specified map is null
     */
    public void putAll(Map<? extends K, Float> m) {
    ...
    public void putAll(FloatValueMap<K> m) {
        int numKeysToBeAdded = m.size();
        if (numKeysToBeAdded == 0)
            return;
    ...
    public float remove(Object key, float defaultRetVal) {
    ...
    final Entry<K> removeMapping(Object o) {
        if (size == 0 || !(o instanceof FloatValueMap.Entry))
            return null;

        @SuppressWarnings("unchecked")
        FloatValueMap.Entry<K> entry = (FloatValueMap.Entry<K>) o;
    ...
    @SuppressWarnings("unchecked")
    public Object clone() {
        FloatValueHashMap<K> result = null;
        try {
            result = (FloatValueHashMap<K>) super.clone();
        ...
        if (result.table != EMPTY_TABLE) {
            result.inflateTable(Math.min((int) Math.min(size * Math.min(1 / loadFactor, 4.0f),
                    // we have limits...
                    FloatValueHashMap.MAXIMUM_CAPACITY), table.length));
        }
    ...
    static class Entry<K> implements FloatValueMap.Entry<K> {
        final K key;
        float value;
        Entry<K> next;
        int hash;
        ...
        public final boolean equals(Object o) {
            if (!(o instanceof FloatValueMap.Entry))
                return false;
            @SuppressWarnings("rawtypes")
            FloatValueMap.Entry e = (FloatValueMap.Entry) o;
            Object k1 = getKey();
            Object k2 = e.getKey();
        ...
        public final int hashCode() {
            return Objects.hashCode(getKey()) ^ Helper.floatHash(getValue());
        }
         ...
         * HashMap.
         */
        void recordAccess(FloatValueHashMap<K> m) {
        }

        /**
         * This method is invoked whenever the entry is removed from the table.
         */
        void recordRemoval(FloatValueHashMap<K> m) {
        }
    }
    ...
    private abstract class HashIterator {
        ...
        public void remove() {
            ...
            current = null;
            FloatValueHashMap.this.removeEntryForKey(k);
            expectedModCount = modCount;
        }
    }

    private final class ValueIterator extends HashIterator implements FloatIterator {
        public float next() {
    ...
    private final class EntryIterator extends HashIterator
            implements Iterator<FloatValueMap.Entry<K>> {
        public FloatValueHashMap.Entry<K> next() {
            return nextEntry();
        }
    }
    ...
    FloatIterator newValueIterator() {
        return new ValueIterator();
    }

    Iterator<FloatValueMap.Entry<K>> newEntryIterator() {
        return new EntryIterator();
    }

    // Views

    private transient Set<FloatValueMap.Entry<K>> entrySet = null;
    private transient volatile Set<K> keySet = null;
    private transient volatile FloatCollection values = null;
    ...
    private final class KeySet extends AbstractSet<K> {
        ...
        public boolean remove(Object o) {
            return FloatValueHashMap.this.removeEntryForKey(o) != null;
        }
     ...
     */
    public Set<FloatValueMap.Entry<K>> entrySet() {
        return entrySet0();
    }

    private Set<FloatValueMap.Entry<K>> entrySet0() {
        Set<FloatValueMap.Entry<K>> es = entrySet;
        return es != null ? es : (entrySet = new EntrySet());
    }

    private final class EntrySet extends AbstractSet<FloatValueMap.Entry<K>> {
        public Iterator<FloatValueMap.Entry<K>> iterator() {
        ...
        public boolean contains(Object o) {
            if (!(o instanceof FloatValueMap.Entry))
                return false;
            @SuppressWarnings("unchecked")
            FloatValueMap.Entry<K> e = (FloatValueMap.Entry<K>) o;
    ...
    private void writeObject(java.io.ObjectOutputStream s) throws IOException {
        ...
        if (size > 0) {
            for (FloatValueMap.Entry<K> e : entrySet0()) {
    ...
    private static final long serialVersionUID = 4508889189065033581L;
