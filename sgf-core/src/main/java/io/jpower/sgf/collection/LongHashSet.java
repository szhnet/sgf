...
public class LongHashSet extends AbstractLongSet
    ...
    private static final long serialVersionUID = 2642506864979232326L;
     ...
     */
    public LongHashSet(Collection<Long> c) {
        map = new LongHashMap<>(Math.max((int) (c.size() / .75f) + 1, 16));
        addAll(c);
    }

    public LongHashSet(long[] a) {
        map = new LongHashMap<>(Math.max((int) (a.length / .75f) + 1, 16));
        addAll(a);
    }

    public LongHashSet(LongSet s) {
        map = new LongHashMap<>(Math.max((int) (s.size() / .75f) + 1, 16));
        addAll(s);
    }
    ...
    public LongHashSet(int initialCapacity, float loadFactor) {
        map = new LongHashMap<>(initialCapacity, loadFactor);
    }

    public LongHashSet(int initialCapacity) {
        map = new LongHashMap<>(initialCapacity);
    }
    ...
    public Object clone() {
        try {
            ...
            newSet.map = (LongHashMap<Object>) map.clone();
            return newSet;
    ...
    private void writeObject(java.io.ObjectOutputStream s) throws java.io.IOException {
        ...
        for (LongMap.Entry<Object> e : map.entrySet())
    ...
    private void readObject(java.io.ObjectInputStream s)
        ...
        map = new LongHashMap<Object>(capacity, loadFactor);
