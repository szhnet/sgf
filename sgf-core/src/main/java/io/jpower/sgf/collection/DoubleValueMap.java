 ...
 */
public interface DoubleValueMap<K> {
    ...
    double get(Object key, double defaultRetVal);
    ...
    double put(K key, double value, double defaultRetVal);
    ...
    double remove(Object key, double defaultRetVal);
     ...
     * {@link #put(K, double) put(k, v)} on this map once for each mapping from
     ...
     * @throws ClassCastException            if the class of a key or value in the specified map prevents
     ...
     */
    void putAll(Map<? extends K, Double> m);

    void putAll(DoubleValueMap<K> m);
     ...
     */
    Set<DoubleValueMap.Entry<K>> entrySet();
