...
public abstract class AbstractDoubleCollection implements DoubleCollection {
     ...
     */
    private static double[] finishToArray(double[] r, DoubleIterator itr) {
        int i = r.length;
        while (itr.hasNext()) {
            int cap = r.length;
            if (i == cap) {
    ...
    public boolean containsAll(DoubleCollection c) {
        for (DoubleIterator itr = c.iterator(); itr.hasNext(); ) {
    ...
    private boolean contains(double[] array, double e) {
        for (double a : array) {
            if (a == e) {
                return true;
            }
