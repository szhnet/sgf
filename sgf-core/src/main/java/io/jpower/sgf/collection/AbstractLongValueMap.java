...
public abstract class AbstractLongValueMap<K> implements LongValueMap<K> {
     ...
     * implicit.)
     */
    protected AbstractLongValueMap() {
    }
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof LongValueMap))
            return false;
        @SuppressWarnings("unchecked")
        LongValueMap<K> m = (LongValueMap<K>) o;
        if (m.size() != size())
            return false;

        try {
            Iterator<LongValueMap.Entry<K>> i = entrySet().iterator();
            while (i.hasNext()) {
                LongValueMap.Entry<K> e = i.next();
                K key = e.getKey();
    ...
    public static class SimpleEntry<K> implements LongValueMap.Entry<K>, java.io.Serializable {

        private static final long serialVersionUID = 4266050622909084839L;
         ...
         */
        public SimpleEntry(K key, long value) {
            this.key = key;
            this.value = value;
        }
         ...
         */
        public int hashCode() {
            return (key == null ? 0 : key.hashCode()) ^ Helper.longHash(value);
        }
