...
public class LongHashMap<V> extends AbstractLongMap<V>
     ...
     */
    public LongHashMap(Map<Long, ? extends V> m) {
    ...
    public LongHashMap(LongMap<? extends V> m) {
        this(Math.max((int) (m.size() / DEFAULT_LOAD_FACTOR) + 1, DEFAULT_INITIAL_CAPACITY),
                DEFAULT_LOAD_FACTOR);
        inflateTable(threshold);
    ...
    private void putAllForCreate(Map<Long, ? extends V> m) {
        for (Map.Entry<Long, ? extends V> e : m.entrySet())
            putForCreate(e.getKey(), e.getValue());
    }

    private void putAllForCreate(LongMap<? extends V> m) {
    ...
    final Entry<V> removeMapping(Object o) {
        if (size == 0 || !(o instanceof LongMap.Entry))
            return null;
    ...
    public Object clone() {
        ...
        if (result.table != EMPTY_TABLE) {
            result.inflateTable(Math.min((int) Math.min(size * Math.min(1 / loadFactor, 4.0f),
                    // we have limits...
                    IntHashMap.MAXIMUM_CAPACITY), table.length));
        }
    ...
    static class Entry<V> implements LongMap.Entry<V> {
        final long key;
        V value;
        Entry<V> next;
        int hash;
        ...
        public final boolean equals(Object o) {
            if (!(o instanceof LongMap.Entry))
                return false;
            @SuppressWarnings("unchecked")
            LongMap.Entry<V> e = (LongMap.Entry<V>) o;
            long k1 = getKey();
            long k2 = e.getKey();
            if (k1 == k2) {
                Object v1 = getValue();
        ...
        public final int hashCode() {
            return Helper.longHash(getKey()) ^ Objects.hashCode(getValue());
        }
         ...
         * HashMap.
         */
        void recordAccess(LongHashMap<V> m) {
        }

        /**
         * This method is invoked whenever the entry is removed from the table.
         */
        void recordRemoval(LongHashMap<V> m) {
        }
    }
    ...
    private final class KeyIterator extends HashIterator<Long> implements LongIterator {
        public long next() {
            return nextEntry().getKey();
        }
    }

    private final class EntryIterator extends HashIterator<LongMap.Entry<V>>
    ...
    Iterator<LongMap.Entry<V>> newEntryIterator() {
        return new EntryIterator();
    }

    // Views

    private transient Set<LongMap.Entry<V>> entrySet = null;
    private transient volatile LongSet keySet = null;
    ...
    private Set<LongMap.Entry<V>> entrySet0() {
        Set<LongMap.Entry<V>> es = entrySet;
        return es != null ? es : (entrySet = new EntrySet());
    }

    private final class EntrySet extends AbstractSet<LongMap.Entry<V>> {
    ...
    private void writeObject(java.io.ObjectOutputStream s) throws IOException {
        ...
        if (size > 0) {
            for (LongMap.Entry<V> e : entrySet0()) {
    ...
    private static final long serialVersionUID = 5644313565495575780L;
