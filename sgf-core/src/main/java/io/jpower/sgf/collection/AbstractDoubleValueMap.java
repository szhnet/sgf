...
public abstract class AbstractDoubleValueMap<K> implements DoubleValueMap<K> {
     ...
     * implicit.)
     */
    protected AbstractDoubleValueMap() {
    }
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof DoubleValueMap))
            return false;
        @SuppressWarnings("unchecked")
        DoubleValueMap<K> m = (DoubleValueMap<K>) o;
        if (m.size() != size())
            return false;

        try {
            Iterator<DoubleValueMap.Entry<K>> i = entrySet().iterator();
            while (i.hasNext()) {
                DoubleValueMap.Entry<K> e = i.next();
                K key = e.getKey();
    ...
    public static class SimpleEntry<K> implements DoubleValueMap.Entry<K>, java.io.Serializable {

        private static final long serialVersionUID = 10744192716200896L;
         ...
         */
        public SimpleEntry(K key, double value) {
            this.key = key;
            this.value = value;
        }
         ...
         */
        public int hashCode() {
            return (key == null ? 0 : key.hashCode()) ^ Helper.doubleHash(value);
        }
