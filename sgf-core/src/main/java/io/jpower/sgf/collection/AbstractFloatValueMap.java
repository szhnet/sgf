...
public abstract class AbstractFloatValueMap<K> implements FloatValueMap<K> {
     ...
     * implicit.)
     */
    protected AbstractFloatValueMap() {
    }
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof FloatValueMap))
            return false;
        @SuppressWarnings("unchecked")
        FloatValueMap<K> m = (FloatValueMap<K>) o;
        if (m.size() != size())
            return false;

        try {
            Iterator<FloatValueMap.Entry<K>> i = entrySet().iterator();
            while (i.hasNext()) {
                FloatValueMap.Entry<K> e = i.next();
                K key = e.getKey();
    ...
    public static class SimpleEntry<K> implements FloatValueMap.Entry<K>, java.io.Serializable {
         ...
         */
        public int hashCode() {
            return (key == null ? 0 : key.hashCode()) ^ Helper.floatHash(value);
        }
